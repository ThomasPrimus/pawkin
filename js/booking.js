...
function openBooking(){
  ...
  $('sheet').innerHTML = `
    ...
    <div style="display:flex;gap:10px">
      <div class="fld" style="flex:1"><label>Von</label><input id="bFrom" type="date" min="${heuteISO()}"></div>
      <div class="fld" style="flex:1"><label>Bis</label><input id="bTo" type="date" min="${heuteISO()}"></div>
    </div>
    <p id="bSpan" style="font-size:11.5px;color:var(--muted);margin:-6px 0 10px">Leer lassen = nach Absprache.</p>
  ...
  // Bis-Datum darf nicht vor Von liegen – das fängt die Datenbank zwar ab,
  // aber der Fehler soll gar nicht erst entstehen.
  const spann = () => {
    const von = $('bFrom').value, bis = $('bTo').value;
    if(von) $('bTo').min = von;
    if(von && bis && bis < von){ $('bTo').value = von; }
    $('bSpan').textContent = von
      ? zeitraumText({starts_on: von, ends_on: $('bTo').value||von})
      : 'Leer lassen = nach Absprache.';
  };
  $('bFrom').oninput = spann; $('bTo').oninput = spann;
...
async function confirmBooking(){
  ...
  const von = $('bFrom').value || null;
  const bis = $('bTo').value || von;
  // date_text bleibt die Anzeige und wird aus den Daten erzeugt, damit
  // Liste und Chat denselben Wortlaut zeigen.
  const dates = von ? zeitraumText({starts_on: von, ends_on: bis}) : 'nach Absprache';
  $('bSend').textContent='Sende…';
  const {data:bk, error} = await sb.from('bookings').insert({
    owner_id: me.id, sitter_id: s.id, pet_id: petId, service: $('bSvc').value,
    date_text: dates, starts_on: von, ends_on: bis, message: $('bMsg').value
  }).select().single();
